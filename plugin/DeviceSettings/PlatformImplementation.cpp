...
public:
    ...
    {
	LOGWARN("Set %s audio configuration to enable = %d", audioConfig.c_str(), enable);
	try
        {
    ...
    uint32_t GetApplicationAudioConfig(const std::string& audioConfig, bool &enable /* @out */) override
    {
	LOGWARN("Get %s audio configuration", audioConfig.c_str());
	try
        {
            device::Host::getInstance().getApplicationAudioConfig(audioConfig, &enable);
	    LOGWARN("%s audio config enabled =%d", audioConfig.c_str(), enable);
        }
        catch (const device::Exception& err)
        {
            TRACE(Trace::Error, (_T("Exception during DeviceSetting library call. code = %d message = %s"), err.getCode(), err.what()));
    ...
    uint32_t GetApplicationAudioConfigList(IAudioConfigListIterator*&  applicationAudioConfigs) const override
    {
	std::vector<std::string> configList;
	    ...
	    for (std::vector<std::string>::iterator it = configList.begin(); it != configList.end(); it++)
               LOGWARN("audio config = %s", it->c_str());
        }
        catch (const device::Exception& err)
        {
            TRACE(Trace::Error, (_T("Exception during DeviceSetting library call. code = %d message = %s"), err.getCode(), err.what()));
